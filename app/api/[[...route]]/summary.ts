...
.get(
    ...
    async(c)=>{
        ...
        .select({
            ...
            `.mapWith(Number),
            expenses: sql`
                SUM(CASE WHEN ${transactions.amount} < 0 THEN ABS(${transactions.amount}) ELSE 0 END)
            `.mapWith(Number)
        })
        .from(transactions)
        .innerJoin(
